...
import {
  loadPersonaSections,
  wrapBusinessContext,
} from "../../../lungmat/persona.js";
import { getPersonaPath } from "../../../lungmat/config.js";
...
function lungmatPromptOverrides() {
  if (!getPersonaPath()) return null;
  const s = loadPersonaSections();
  if (!s.businessContext && !s.postContentRules) return null;
  return {
    businessContext: s.businessContext
      ? wrapBusinessContext(s.businessContext)
      : undefined,
    tweetExamples: s.tweetExamples
      ? `<examples>\n${s.tweetExamples}\n</examples>`
      : undefined,
    postStructureInstructions: s.postStructure,
    postContentRules: s.postContentRules
      ? `- ${s.postContentRules.replace(/\n/g, "\n- ")}`
      : undefined,
  };
}

export function getPrompts() {
  const lungmat = lungmatPromptOverrides();

  // NOTE: you should likely not have this set, unless you want to use the LangChain prompts
  if (useLangChainPrompts()) {
    return {
      businessContext:
        lungmat?.businessContext ?? LANGCHAIN_BUSINESS_CONTEXT,
      tweetExamples: lungmat?.tweetExamples ?? LANGCHAIN_TWEET_EXAMPLES,
      postStructureInstructions:
        lungmat?.postStructureInstructions ??
        LANGCHAIN_POST_STRUCTURE_INSTRUCTIONS,
      postContentRules:
        lungmat?.postContentRules ?? LANGCHAIN_POST_CONTENT_RULES,
      contentValidationPrompt: LANGCHAIN_CONTENT_VALIDATION_PROMPT,
    };
  }

  return {
    businessContext: lungmat?.businessContext ?? BUSINESS_CONTEXT,
    tweetExamples: lungmat?.tweetExamples ?? TWEET_EXAMPLES,
    postStructureInstructions:
      lungmat?.postStructureInstructions ?? POST_STRUCTURE_INSTRUCTIONS,
    postContentRules: lungmat?.postContentRules ?? POST_CONTENT_RULES,
