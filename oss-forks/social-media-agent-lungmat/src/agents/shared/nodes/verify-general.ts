...
import { scrapeUrlMarkdown } from "../../../lungmat/firecrawl-scrape.js";
...
async function getUrlContentsFunc(url: string): Promise<UrlContents> {
  try {
    const scraped = await scrapeUrlMarkdown(url, {
    ...
    return {
      ...
      imageUrls: getImagesFromFireCrawlMetadata(doc.metadata) || [],
    };
  } catch (firecrawlErr) {
    console.warn("[lungmat] Firecrawl scrape failed, fallback getPageText:", firecrawlErr);
  }
